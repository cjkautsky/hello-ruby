...
puts "Currently it is #{weather_data[:current][:temperature]} degrees and #{weather_data[:current][:conditions].downcase}. Tomorrow it will be #{weather_data[:forecast][0][:temperature]} and #{weather_data[:forecast][0][:conditions].downcase}. 
The next day we will see #{weather_data[:forecast][1][:conditions].downcase} skies, with a temperature of #{weather_data[:forecast][1][:temperature]}."
